...
export async function POST(req: NextRequest) {
  try {
    ...
    const existingRole = await EmployeeRoleModel.findOne({
      roleName: parsedData.roleName,
    });

    console.log("Existing role check:", existingRole); // Debug log
    ...
    // const newRole = await EmployeeRoleModel.create({
    //   roleName: parsedData.roleName,
    ...
    const newRole = await EmployeeRoleModel.create({
      ...
      permissions: Object.fromEntries(
        Object.entries(parsedData.permissions).map(([key, value]) => [
          key,
          {
            All: value.All ?? false,
            Add: value.Add ?? false,
    ...
    console.log("Created new role:", newRole); // Debug log
...
export async function GET() {
  try {
    ...
    const roles = await EmployeeRoleModel.find().lean();

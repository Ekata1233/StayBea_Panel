...
const plans = [
  {
    ...
    net: '+₹34.33 L',
    revenueValue: 3754162,
    barGradient: 'from-rose-400/80 via-rose-500/90 to-rose-700',
    revenue: '₹37.54 L',
    perkBurn: '₹3.21 L',
    perkBreak: '(₹3.21 L perks + ₹0 coins)',
    margin: '91%',
    arpu: '₹446',
    termMix: '1 month ₹499/mo × 45% · 3 months ₹433/mo × 30% · 6 months ₹400/mo × 18% · Forever ₹278/mo × 7%',
    burnPerUser: '₹38',
    perkLine: '🚀 Boosts 1/wk → ₹17 · ❤️ Compliments 3/wk → ₹21 at 40% redemption · roses excluded (always paid)',
    summary: '₹37,54,162 revenue − ₹3,21,083 burn = ₹34,33,080 net / mo · 91% margin',
    summarySub: 'across 8,420 subscribers',
  },
  {
    name: 'VIP',
    ...
    net: '+₹48.93 L',
    revenueValue: 5502988,
    barGradient: 'from-amber-400/80 via-amber-500/90 to-amber-800',
    revenue: '₹55.03 L',
    perkBurn: '₹6.10 L',
    perkBreak: '(₹3.62 L perks + ₹2.48 L coins)',
    margin: '89%',
    arpu: '₹1,847',
    termMix: '1 month ₹1,999/mo × 45% · 3 months ₹1,833/mo × 30% · 6 months ₹1,667/mo × 18% · Forever ₹1,389/mo × 7%',
    burnPerUser: '₹205',
    perkLine: '🚀 Boosts 3/wk → ₹52 · ❤️ Compliments 10/wk → ₹69 · 🪙 Welcome coins ₹83 at 40% redemption · roses excluded (always paid)',
    summary: '₹55,02,988 revenue − ₹6,09,907 burn = ₹48,93,081 net / mo · 89% margin',
    summarySub: 'across 2,980 subscribers',
  },
  {
    name: 'VIP Elite',
    ...
    net: '+₹87.72 L',
    revenueValue: 9578177,
    barGradient: 'from-rose-700/80 via-rose-900/90 to-black',
    revenue: '₹95.78 L',
    perkBurn: '₹8.07 L',
    perkBreak: '(₹3.10 L perks + ₹4.97 L coins)',
    margin: '92%',
    arpu: '₹6,428',
    termMix: '1 month ₹6,999/mo × 45% · 3 months ₹6,333/mo × 30% · 6 months ₹5,500/mo × 18% · Forever ₹5,556/mo × 7%',
    burnPerUser: '₹541',
    perkLine: '🚀 Boosts 6/wk* → ₹104 · ❤️ Compliments 15/wk* → ₹104 · 🪙 Welcome coins ₹333 at 40% redemption · roses excluded (always paid)',
    summary: '₹95,78,177 revenue − ₹8,06,587 burn = ₹87,71,591 net / mo · 92% margin',
    summarySub: 'across 1,490 subscribers',
  },
]

const maxRevenue = Math.max(...plans.map((p) => p.revenueValue))
...
function RevenueBar({
  ...
  // bar fill width — revenue proportional
  const fillWidth = Math.max((revenueValue / maxRevenue) * 100, 4)
  // pink pill width — revenue proportional (max ~45% of bar), text se chhoti kabhi nahi
  const pillWidth = (revenueValue / maxRevenue) * 45

  return (
    <div className="relative h-8 w-full rounded-full bg-rose-50/60 ring-1 ring-inset ring-rose-100/70">
      {/* fill */}
      <div
        className={`absolute inset-y-0 left-0 overflow-hidden rounded-full bg-gradient-to-r ${gradient} transition-[width] duration-700 ease-out`}
        style={{ width: `${fillWidth}%` }}
      />
      {/* pink revenue pill — width revenue ke saath badhti/ghatti hai */}
      <div
        className="absolute left-1 top-1/2 z-10 flex h-6 -translate-y-1/2 items-center whitespace-nowrap rounded-full bg-rose-900/60 pl-2.5 pr-3 ring-1 ring-inset ring-rose-300/50 backdrop-blur-sm transition-[width] duration-700 ease-out"
        style={{ width: `${pillWidth}%`, minWidth: 'fit-content' }}
      >
        <span className="text-xs font-medium text-white">Revenue {revenueLabel}</span>
      </div>
    </div>
  )
}
/* --------------------------------- page ----------------------------------- */
export default function PackageRevenue() {
  const [openPlan, setOpenPlan] = React.useState<string | null>(null)

  const toggle = (name: string) =>
    setOpenPlan((prev) => (prev === name ? null : name))
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
                        <span className="flex items-center gap-1.5">
                          ...
                          <span className="text-gray-600">
                            Revenue{' '}
                            <span className="font-semibold text-gray-800">{p.revenue}</span>
                          </span>
                        </span>
                        <span className="flex items-center gap-1.5">
                          ...
                          <span className="text-gray-600">
                            Perk burn{' '}
                            <span className="font-semibold text-gray-800">{p.perkBurn}</span>{' '}
                    ...
                    {isOpen && (
                      <div className="mt-4 space-y-4">
                        ...
                        <div className="border-t border-gray-100 pt-4">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm font-bold text-rose-500">
                              {p.arpu}{' '}
                              <span className="text-xs font-normal text-gray-400">/mo ARPU</span>
                            </span>
                          </div>
                          <p className="mt-2 text-xs text-gray-500">{p.termMix}</p>
                        </div>
                        ...
                        <div className="border-t border-gray-100 pt-4">
                          ...
                          <p className="mt-2 text-xs text-gray-500">{p.perkLine}</p>
                        </div>

                        {/* summary strip */}
                        <div className="rounded-xl bg-[#f7f5f2] px-4 py-3 text-center">
                          <p className="text-xs font-semibold text-emerald-700">{p.summary}</p>
                          <p className="mt-0.5 text-[11px] text-gray-400">{p.summarySub}</p>
                        </div>
                      </div>
                    )}
                  </div>

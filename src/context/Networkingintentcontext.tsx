...
const SCREEN = "NETWORKING_INTENT";
...
const NetworkingIntentContext = createContext<Ctx | null>(null);

export function NetworkingIntentProvider({
  children,
}: {
  ...
  return (
    <NetworkingIntentContext.Provider
      value={{
        fields,
        loading,
        error,
...
export function useNetworkingIntent() {
  const ctx = useContext(NetworkingIntentContext);

  if (!ctx) {
    throw new Error(
      "useNetworkingIntent must be used within NetworkingIntentProvider"
    );
  }
